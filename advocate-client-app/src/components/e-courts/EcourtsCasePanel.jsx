...
export default function EcourtsCasePanel({ cnr, embedded = true }) {
  ...
  /** From GET /api/partner/case/{cnr} → `data.courtCaseData` (snake_case fallback). */
  ...
  /**
   * 1) Case list comes from GET /api/partner/case/{cnr} (`court` → judgment/interim rows).
   * 2) Each row’s `orderUrl` is the sole `filename` for GET /api/partner/case/{cnr}/order/{filename}.
   */
