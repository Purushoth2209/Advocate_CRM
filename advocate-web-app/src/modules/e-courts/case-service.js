...
/**
 * Partner API expects the **filename segment** only (e.g. `order-1.pdf`), matching
 * `judgmentOrders[].orderUrl` / `interimOrders[].orderUrl` from GET /case/{cnr}.
 * Some responses may return a full URL or path — normalize to the last path segment after `/order/` if present.
...
/**
 * Download order PDF — **step 2** after `getCaseByCnr` (GET case details).
 * **GET** (no body). Path: `/case/{cnr}/order/{filename}` with `Authorization: Bearer <token>`.
 * `filename` must be the **exact** `orderUrl` string from `GET /case/{cnr}` response:
 * `data.courtCaseData.judgmentOrders[].orderUrl` or `data.courtCaseData.interimOrders[].orderUrl`
 * (e.g. `order-1.pdf`). Do not guess filenames; they come only from that case-details call.
 * @param {string} cnr — same CNR as the case detail request
 * @param {string} filename — copied from `orderUrl` on an order row from case detail
...
export async function downloadOrderPdf(cnr, filename) {
  ...
  /** Same family as EcourtsIndia reference client: Bearer + JSON accept (errors are JSON; success is PDF). */
  const headers = {
  ...
  const attempt = async () => {
    ...
    /** Some APIs return 200 + JSON error body instead of 4xx — don't treat as PDF. */
